...
const AXES: { key: keyof PerspectivesData; label: string; color: string; desc: string }[] = [
  { key: "people",  label: "People",  color: "#10b981", desc: "Stakeholders, org change, human-in-the-loop" },
  { key: "systems", label: "Systems", color: "#8b5cf6", desc: "Standards, interoperability, MIMs compliance" },
  { key: "planet",  label: "Planet",  color: "#3b82f6", desc: "Spatial awareness, environment, GIS/BIM/CIM" },
  { key: "ai",      label: "AI",      color: "#f59e0b", desc: "Agent autonomy, explainability, trust, LPI compliance" },
];
...
export default function PerspectivesRadar({ data, size = 280 }: PerspectivesRadarProps) {
  ...
  const numRings = 5;
  // Diamond: 4 axes evenly spaced at 90° each, starting at top (0°)
  ...
  return (
    <div ref={ref} className="glass rounded-2xl p-6 border border-emerald-500/20">
      ...
      <div className="flex items-center justify-center">
        <svg
          ...
          aria-label="Four Perspectives radar chart — People, Systems, Planet, AI"
          role="img"
        >
          {/* Grid rings */}
          {rings.map(({ points }, i) => (
